...
void gotoxy (int x, int y)		//function that sets the cursor position for simply to move the cursor on your monitor screen wherever desired.
{
...
void login()		//function that display login screen
{
	...
	printf("\n\t\t\tPASSWORD: ");
	scanf("%s",&p);
	if((strcmp(u,"aaa")==0) && (strcmp(p,"bbb")==0))
	{
		printf("\n\n\t\t login successful");
